...
Key はカラムがインデックス付けされているかどうかを示す。Key の値には PRI 、UNI、MUL の 3 種があり、カラムに対して複数の Key 値が適用される場合は、前述の順序で表示される。今回は PRI と MUL を使用するため、その 2 つについて述べる。
...
車が最後に通信してきたときから、n 秒通信がないと 1 増加する。3 になると通信切断による異常状態と判断され、対象の車の状態が 5 になる。
...
管理者の開始時刻。他のテーブルの startAt とは関係ない。

#### 4.2.5. endAt ( 終了時刻 )

管理者の終了時刻。該当の管理者が利用中だと NULL になる。

これが NULL の時、開始時刻と比較して 1 時間を過ぎると管理者が強制的にログアウト状態になり、その時の時刻が更新される。
...
登録された経路の名前。経路を一意に識別するため名前被りは禁止されている。
